...
class PegasusDecoderLayer(nn.Module):
    ...
    def forward(
        ...
        # print('decoder_ouput',outputs[0].shape)
    ...
    def forward(
        ...
        Args:
            ...
            return_dict (:obj:`bool`, `optional`):
                Whether or not to return a :class:`~transformers.file_utils.ModelOutput` instead of a plain tuple.
        """
        # print('config.encoder_layers',self.config.encoder_layers)
        ...
        for idx, encoder_layer in enumerate(self.layers):
            if output_hidden_states:
                ...
                # print('idx',idx,hidden_states.shape,len(encoder_states))
        ...
        # print('hidden_states',len(encoder_states))
        # for i in range(len(encoder_states)):
        #     print('i',i, encoder_states[i].shape)
...
class PegasusDecoder(PegasusPreTrainedModel):
    ...
    def forward(
        ...
        for idx, decoder_layer in enumerate(self.layers):
            # print('--------------decoder layer is {} ---------'.format(idx))
            # add LayerDrop (see https://arxiv.org/abs/1909.11556 for description)
            if output_hidden_states:
...
class CustomPegasusModel(PegasusPreTrainedModel):
    ...
    def forward(
        ...
        # print('encoder_outputs',encoder_outputs.keys(),len(encoder_outputs))
...
class PegasusScorer(PegasusPreTrainedModel):
    ...
    def forward(
        ...
        # print('here1')
        # print('pegasusoutputs.keys()',outputs.keys())
        lm_logits = self.lm_head(outputs[0]) + self.final_logits_bias
